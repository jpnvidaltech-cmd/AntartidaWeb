...
export async function sendToWebhook(payload: WebhookPayload): Promise<void> {
  ...
  if (!webhookUrl) {
    console.warn("[webhook] WEBHOOK_URL no está configurada. El mensaje se perderá.");
    // En modo "demo" o sin backend, simplemente simulamos éxito en el log
    console.info("[webhook] MODO SIMULACIÓN: Payload que se habría enviado:", payload);
    return;
  }
